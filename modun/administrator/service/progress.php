...
if ($_REQUEST) {
    ...
    if ($type === 'service_progress') {
        ...
        if ($check_id_available == 0) {
            ...
            if (mysqli_query($conn, "UPDATE table_buff SET buff_status = 'progress' WHERE buff_id = '$buff_id'")) {
                $return['msg'] = 'Trạng thái chờ duyệt cho id ' . $buff_id . ' thành công.';
                die(json_encode($return));
            } else {
                $return['error'] = 1;

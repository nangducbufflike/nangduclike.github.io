...
$result = mysqli_query($conn,"SELECT * FROM table_service WHERE service_slug = 'buff-share-facebook'");
            ...
            <div class="card-header bg-light">
                <div class="row justify-content-between">
                    ...
                    <div class="col-md-auto"><a class="btn btn-falcon-default btn-sm"
                            href="/service/facebook/share/list"><span class="fas fa-list fs--2 mr-1"></span> Danh sách
                            đơn</a>
                    </div>
                </div>
            </div>
                ...
                <form action="" id="form_id" class="user" method="POST" accept-charset="utf-8">
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 2
                            (<b><?=$row['service_price_server_1'];?>₫</b>) (Share bài viết, share người dùng việt thật)) <b
                                class="text-warning">(Hoạt Động)</b></label>
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 3
                            (<b><?=$row['service_price_server_2'];?>₫</b>) (Share bài viết, share giá rẻ) <b
                                class="text-warning">(Hoạt Động)</b></label>


                    
<div class="alert alert-warning alert-dismissible fade show fs--1 text-center" role="alert">
                                 - Các Server Đều Không Bảo Hành Nên Buff Dư Cho Khách Nhé </div>
                    ...
                    <center>
                        <button type="button" class="btn btn-primary btn-rounded me-1 mb-1" id="shareFacebook"
                            onclick="share_facebook();"><i class="fa fa-dollar-sign"></i> Thanh Toán</button>
                    </center>
                </form>
                </p>
            </div>
...
function share_facebook() {
    ...
    $.ajax({
        url: WEBSITE_URL + prefix + 'service/facebook/share/order',
        type: 'POST',
        dataType: 'JSON',
        data: {
        ...
        beforeSend: function() {
            wait('#shareFacebook', false);
        },
        complete: function() {
            wait('#shareFacebook', true, '<i class="fa fa-dollar-sign"></i> Thanh Toán');
        },
        success: (data) => {
            if (data.error) {
                swal(data.msg, "error");

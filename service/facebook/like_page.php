                ...
                <form action="" id="form_id" class="user" method="POST" accept-charset="utf-8">
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 1 (<b><?=$row['service_price'];?>₫</b>)
                            (Like bấm tay, ít tụt, có 1 vài page ko thể tăng like, nên dùng nhé) <b class="text-warning">(ông)</b></label>
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 3
                            (<b><?=$row['service_price_server_2'];?>₫</b>)  (Max 50k, bảo hành 2 tháng) <b class="text-warning">(Hoạt đông)</b></label>
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 4
                            (<b><?=$row['service_price_server_3'];?>₫</b>)  (Max 20k, bảo hành 1 tháng) <b
                                class="text-warning">(Hoạt Động)</b></label>
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 5
                            (<b><?=$row['service_price_server_4'];?>₫</b>) (Lên sau 10-30p Chậm) <b
                                class="text-warning">(Hoạt Động)</b></label>
                                 <div class="alert alert-warning alert-dismissible fade show fs--1 text-center" role="alert">- Tốc độ tăng like page Sv2 và Sv3 trong 1 ngày lên từ 50-300 like! sẽ chạy sau khi mua từ 1-12h sau khi mua.
- Tốc độ tăng like page Sv2 và Sv3 trong 1 ngày lên từ 300-5000 like! sẽ chạy sau khi mua từ 1-12h sau khi mua.
- Riêng đối vớí Sv2 và Sv3 và Sv4 hỗ trợ bảo hành đối với page có số like ban đầu dưới 10k like (riêng đối với id dưới 10k like page có dấu hiệu tụt like không phải của hệ thống sẽ ko hỗ trợ bảo hành!)
- Những máy chủ có bảo hành thì chỉ bảo hành khi tụt trên 70%.
                                  </div>
                    ...
                    <center>
                        <button type="button" class="btn btn-primary btn-rounded me-1 mb-1" id="likepageFacebook"
                            onclick="like_page_facebook();"><i class="fa fa-dollar-sign"></i> Thanh Toán</button>
                    </center>
                </form>
                </p>
            </div>
...
function like_page_facebook() {
    ...
    $.ajax({
        url: WEBSITE_URL + prefix + 'service/facebook/like-page/order',
        type: 'POST',
        dataType: 'JSON',
        data: {
        ...
        beforeSend: function() {
            wait('#likepageFacebook', false);
        },
        complete: function() {
            wait('#likepageFacebook', true, '<i class="fa fa-dollar-sign"></i> Thanh Toán');
        },
        success: (data) => {
            if (data.error) {
                swal(data.msg, "error");

...
$result = mysqli_query($conn,"SELECT * FROM table_service WHERE service_slug = 'buff-follow-facebook'");
            ...
            <div class="card-header bg-light">
                <div class="row justify-content-between">
                    ...
                    <div class="col-md-auto"><a class="btn btn-falcon-default btn-sm"
                            href="/service/facebook/follow/list"><span class="fas fa-list fs--2 mr-1"></span> Danh sách
                            đơn</a>
                    </div>
                </div>
            </div>
                ...
                <form action="" id="form_id" class="user" method="POST" accept-charset="utf-8">
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 1 (<b><?=$row['service_price'];?>₫</b>)
                           (Via việt, max 50k, lên cực nhanh ) <b class="text-warning">(Hoạt Động)</b></label>
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 3
                            (<b><?=$row['service_price_server_2'];?>₫</b>) (Via host, lên nhanh, max 100k, nên dùng, nếu bảo trì lâu thì có thể quay lại vào 12h trưa hôm sau) <b
                                class="text-warning">(Hoạt Động)</b></label>
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 4
                            (<b><?=$row['service_price_server_3'];?>₫</b>) (Via việt, lên nhanh, max 10k, lên cực nhanh)) <b
                                class="text-warning">(Hoạt Động)</b></label>
                        ...
                        <label class="form-check-label" for="buff_server">Server 5
                            (<b><?=$row['service_price_server_4'];?>₫</b>) (Lên sau 10-30p) <b
                                ...
                                <div class="alert alert-warning alert-dismissible fade show fs--1 text-center" role="alert">
                                 - Server 1 2 Sẽ Có Bảo Hành 7 Day </div>
                    ...
                    <center>
                        <button type="button" class="btn btn-primary btn-rounded me-1 mb-1" id="followFacebook"
                            onclick="follow_facebook();"><i class="fa fa-dollar-sign"></i> Thanh Toán</button>
                    </center>
                </form>
                </p>
            </div>
...
function follow_facebook() {
    ...
    $.ajax({
        url: WEBSITE_URL + prefix + 'service/facebook/follow/order',
        type: 'POST',
        dataType: 'JSON',
        data: {
        ...
        beforeSend: function() {
            wait('#followFacebook', false);
        },
        complete: function() {
            wait('#followFacebook', true, '<i class="fa fa-dollar-sign"></i> Thanh Toán');
        },
        success: (data) => {
            if (data.error) {
                swal(data.msg, "error");

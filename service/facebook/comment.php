            ...
            <div class="card-header bg-light">
                <div class="row justify-content-between">
                    ...
                    <div class="col-md-auto"><a class="btn btn-falcon-default btn-sm"
                            href="/service/facebook/comment/list"><span class="fas fa-list fs--2 mr-1"></span> Danh sách đơn</a>
                    </div>
                </div>
            </div>
                ...
                <form action="" id="form_id" class="user" method="POST" accept-charset="utf-8">
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="buff_server">Server 2
                            (<b><?=$row['service_price_server_1'];?>₫</b>) (Mở giới hạn
                            ) (nhận đủ số lượng sẽ tự đóng mở.)</label>

                   
 <div class="alert alert-warning alert-dismissible fade show fs--1 text-center" role="alert">
                                 - Chú Ý Nhập Nội Dung  </div>
                    </div>
                    <?php } ?>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea type="text" id="buff_comment" name="amount" oninput="bill()" class="form-control"
                            rows="7"
                    ...
                    <center>
                        <button type="button" class="btn btn-success btn-rounded me-1 mb-1" id="commentFacebook"
                            onclick="comment_facebook();"><i class="fa fa-dollar-sign"></i> Thanh Toán</button>
                    </center>
                </form>
                </p>
            </div>
...
function comment_facebook() {
    ...
    $.ajax({
        url: WEBSITE_URL + prefix + 'service/facebook/comment/order',
        type: 'POST',
        dataType: 'JSON',
        data: {
        ...
        beforeSend: function() {
            wait('#commentFacebook', false);
        },
        complete: function() {
            wait('#commentFacebook', true, '<i class="fa fa-dollar-sign"></i> Thanh Toán');
        },
        success: (data) => {
            if (data.error) {
                swal(data.msg, "error");
